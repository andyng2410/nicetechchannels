...
def trailing_hashtag_block(value: str, fallback: str = "#NiceTechChannels") -> str:
...
def resolved_hashtag_block(existing_value: str, generated: str) -> str:
    ...
    return generated if block.lower() == "#nicetechchannels" or has_todo_placeholder(block) else block
...
def resolved_youtube_tags(existing_tags: object, generated_tags: list[str]) -> list[str]:
    ...
    meaningful = [tag for tag in cleaned if tag.lower() != "nicetechchannels"]
...
def build_upload_metadata(project: str) -> dict:
    ...
    return {
        ...
        "tags": youtube.get("tags") if isinstance(youtube.get("tags"), list) else ["NiceTechChannels"],
