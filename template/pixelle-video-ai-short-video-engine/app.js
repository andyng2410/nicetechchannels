/* ============================================
   ...
   Pixelle-Video AI Short Video Engine
   ============================================ */
...
// Per-slide audio config - cinematic office-tech vibe
const slideTransitions = ['dramatic', 'sweep', 'bass', 'rise', 'chord', 'minimal'];
const slideReveals = ['sparkle', 'pop', 'blip', 'bubble', 'tick', 'bell'];

// Per-slide script text
const slideScripts = [
  "Pixelle-Video là kiểu repo khiến dân làm content phải giật mình: chỉ nhập một chủ đề, AI tự kéo cả dây chuyền thành video ngắn hoàn chỉnh.",
  "Thay vì mở từng tool riêng lẻ, Pixelle-Video gom cả xưởng sản xuất vào một Web UI. Nó tự viết lời dẫn từ topic, nên bạn không phải ngồi nghĩ kịch bản từ trang trắng. Mỗi câu có thể được lên kế hoạch thành ảnh hoặc video AI, kể cả workflow kiểu WAN 2.1. Rồi hệ thống thêm giọng đọc, nhạc nền và render ra file cuối chỉ bằng một luồng.",
  "Luồng chính của repo này rất rõ: biến một ý tưởng mơ hồ thành timeline có thể render. Bước một là script generation, nơi LLM như Qwen, GPT, DeepSeek hoặc Ollama viết nội dung. Bước hai là image planning, hệ thống chia cảnh và đặt prompt cho từng đoạn. Bước ba là frame-by-frame processing, ảnh hoặc video được đẩy qua ComfyUI workflow. Bước cuối là video composition, ghép visual, voiceover và BGM thành thành phẩm.",
  "Điểm mạnh thật sự nằm ở kiến trúc modular chứ không chỉ ở nút generate cho vui. Bạn có thể chạy self-host với ComfyUI local nếu có GPU. Nếu không muốn dựng máy, RunningHub workflow giúp đẩy phần nặng lên cloud. TTS cũng linh hoạt với Edge-TTS, Index-TTS và nhiều giọng đa ngôn ngữ. Template thì có đủ dọc, ngang, vuông để làm Shorts, YouTube hoặc demo sản phẩm.",
  "Những update gần đây cho thấy Pixelle-Video đang đi từ generator cơ bản sang studio nhỏ. Nó đã có digital human, image-to-video và motion transfer cho các format đang rất hot. Custom media cho phép upload ảnh hoặc video riêng để AI phân tích rồi viết script. Bản v0.1.15 còn thêm tự tải danh sách model LLM và test kết nối API ngay trong UI.",
  "Chốt lại, Pixelle-Video phù hợp với ai muốn thử dựng short video bằng AI mà vẫn kiểm soát được model, workflow và template. Nhưng muốn chạy mượt thì vẫn cần API key, ffmpeg, hoặc một backend ComfyUI/RunningHub đủ khỏe — follow NiceTechChannels để mình bóc tiếp những repo AI đáng thử."
];
...
const defaultPreviewSettings = {
  theme: {
    preset: 'creator-pink-blue',
    variables: themePresets['creator-pink-blue'].variables
  },
  visuals: {
    colorMode: 'dark',
...
function normalizeSlideSettings(settings) {
  ...
  return {
    deletedIds,
    scriptLines: deletedIds.length && scriptLines.length === activeCount ? scriptLines : []
  };
}
...
function applySlideSettings(settings = previewSettings) {
  ...
  slideTransitions.splice(0, slideTransitions.length, ...activeIndexes.map(index => transitionSource[index] || initialSlideTransitions[index] || 'minimal'));
  slideReveals.splice(0, slideReveals.length, ...activeIndexes.map(index => revealSource[index] || initialSlideReveals[index] || 'ping'));
  previewSubtitleCaptions = slideScripts.map(buildPreviewSubtitleCaptions);
...
async function persistProjectScript(statusText = 'Đang tự lưu...', options = {}) {
  ...
  if (syncPreviewSettings) await savePreviewSettings(setStatus);
  setStatus('Đã lưu script-90s.txt và upload-metadata');
}
