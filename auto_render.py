...
def find_system_chromium() -> Path | None:
    ...
    env_path = os.environ.get("NICETECHCHANNELS_BROWSER_PATH") or os.environ.get("ESCBASE_BROWSER_PATH")
...
async def launch_chromium(playwright, args: list[str], *, headless: bool = True):
    ...
    try:
        ...
        if sys.platform.startswith("win"):
            ...
            print("     $env:NICETECHCHANNELS_BROWSER_PATH='C:\\Program Files\\Microsoft\\Edge\\Application\\msedge.exe'")
        raise
...
async def record_video_tab_capture(
    ...
    total_duration = max(timeline[-1]["time"] + 0.5, voiceover_duration + 0.5)
    capture_title = f"NICETECHCHANNELS_TAB_CAPTURE_{int(time.time())}"
