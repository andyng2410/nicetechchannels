...
function Install-PortableFfmpeg {
    ...
    $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("nicetechchannels-ffmpeg-" + [Guid]::NewGuid().ToString("N"))
...
function Test-PlaywrightRuntime {
    $browser = Find-SystemChromium
    if ($browser) {
        $env:NICETECHCHANNELS_BROWSER_PATH = $browser
    } else {
        Remove-Item Env:NICETECHCHANNELS_BROWSER_PATH -ErrorAction SilentlyContinue
    }
...
async def main():
    browser_path = os.environ.get("NICETECHCHANNELS_BROWSER_PATH")
    launch_options = {
        "headless": True,
    ...
    async with async_playwright() as p:
        ...
        context = await browser.new_context(
            ...
            record_video_dir=tempfile.mkdtemp(prefix="nicetechchannels-playwright-"),
            record_video_size={"width": 320, "height": 180},
        )
        page = await context.new_page()
        await page.goto("data:text/html,<html><body>nicetechchannels ok</body></html>")
        await context.close()
        await browser.close()
